...
3. Open a trip → date, shipper, city, line haul, detention, extra pay, and reefer fuel. The trip page does not show a Nickey baseline card. Truck lease, insurance, IFTA, fuel, and truck wash are **not** on the trip.
4. **Save actuals** — variance and flag recompute against the tolerance band using **trip pay**. When line haul (`actualPay`) is filled in, that save always publishes it as Nickey **Current Baseline**. Detention, extra, reefer, and week gross are not the baseline. There is no checkbox and no separate send button.
5. On **Week**, the bottom shows **Gross**, **Deductions**, and **Net** once weekly totals exist. A week with trips but no totals yet says **Pending deductions** and shows gross only. Tap **Enter Weekly Totals** — the form copies last week, and an edited number is highlighted. Week gross stays on the pay sheet. The week page also shows the current Nickey baseline (the latest saved line haul).
